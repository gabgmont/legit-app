        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-400">
              {searchQuery ? `No products match "${searchQuery}"` : "Your collection is empty"}
            </p>
            {searchQuery ? (
              <button onClick={() => setSearchQuery("")} className="mt-4 px-4 py-2 bg-[#4169e1] rounded-md">
                Clear Search
              </button>
            ) : (
              <button onClick={handleAddItem} className="mt-4 px-4 py-2 bg-[#4169e1] rounded-md">
                Add Your First Item
              </button>
            )}
          </div>
